...
@section('title', 'تعديل كود خصم - سوق')
@section('page-title', 'تعديل: ' . $promoCode->code)
...
<form action="{{ route('admin.promo-codes.update', $promoCode) }}" method="POST">
    ...
    <div class="bg-white rounded-xl shadow p-6 mb-6">
        <div class="grid md:grid-cols-2 gap-4 mb-4">
            ...
            <div>
                ...
                <select name="type" class="w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-accent focus:outline-none">
                    <option value="percentage" {{ old('type', $promoCode->type) == 'percentage' ? 'selected' : '' }}>نسبة مئوية (%)</option>
                    <option value="fixed" {{ old('type', $promoCode->type) == 'fixed' ? 'selected' : '' }}>مبلغ ثابت (₪)</option>
                </select>
            </div>

            <div>
                <label class="block text-sm font-medium mb-1">قيمة الخصم</label>
                <input type="number" step="0.01" name="value" value="{{ old('value', $promoCode->value) }}"
            ...
            <div>
                ...
                <input type="number" step="0.01" name="min_order_amount" value="{{ old('min_order_amount', $promoCode->min_order_amount) }}"
                    class="w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-accent focus:outline-none">
            </div>
            ...
            <div>
                ...
                <input type="number" name="usage_limit" value="{{ old('usage_limit', $promoCode->usage_limit) }}"
                    class="w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-accent focus:outline-none">
                <p class="text-xs text-gray-400 mt-1">تم استخدامه {{ $promoCode->used_count }} مرة حتى الآن</p>
            </div>

            <div>
                <label class="block text-sm font-medium mb-1">تاريخ البدء (اختياري)</label>
                <input type="datetime-local" name="starts_at"
                       value="{{ old('starts_at', $promoCode->starts_at?->format('Y-m-d\TH:i')) }}"
                       class="w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-accent focus:outline-none">
            </div>
            <div>
                <label class="block text-sm font-medium mb-1">تاريخ الانتهاء (اختياري)</label>
                <input type="datetime-local" name="expires_at"
                       value="{{ old('expires_at', $promoCode->expires_at?->format('Y-m-d\TH:i')) }}"
                       class="w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-accent focus:outline-none">
        ...
        <label class="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $promoCode->is_active) ? 'checked' : '' }} class="cursor-pointer">
